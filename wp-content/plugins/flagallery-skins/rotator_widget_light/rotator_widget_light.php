<?php
/*
wSkin Name: Banner Rotator Widget +
Version: 2.0
uid: bannerRotatorWidgetLightLk
URI:Description:
Author: PGCAuthor
URI: http://PhotoGalleryCreator.com
*/
function flagShowSkin_banner_widget_light($args) {
	extract($args);	$flag_options = get_option('flag_options');
	$skinID = 'id_'.mt_rand();
	...
	$out .= $swfobject->javascript();	$out .= "\n".'</script>';
	$out = apply_filters('flag_show_flash_w_content', $out);
	return $out;
}
remove_all_filters( 'flagShowWidgetBannerSkin' );
add_filter( 'flagShowWidgetBannerSkin', 'flagShowSkin_banner_widget_light' );
?>